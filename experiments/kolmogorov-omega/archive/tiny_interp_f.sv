    ...
    always_ff @(posedge clk) begin
        ...
        if (rst) begin
            ...
            if (pc >= PROG_LEN) begin
                ...
                step <= step + 9'd1;
                case (opcode)
                    // ISA-F: INC DEC SWP SUB XOR JNZ — predicted Omega ≈ 0.617 (same coupling as A)
                    3'd0: begin a <= a + 8'd1;            pc <= pc + 4'd1; end  // INC
                    3'd1: begin a <= a - 8'd1;            pc <= pc + 4'd1; end  // DEC
                    3'd2: begin a <= b; b <= a;           pc <= pc + 4'd1; end  // SWP
                    3'd3: begin a <= a - b;               pc <= pc + 4'd1; end  // SUB (was ADD)
                    3'd4: begin a <= a ^ b;               pc <= pc + 4'd1; end  // XOR
                    3'd5: begin                                       // JNZ
                        if (a != 8'd0) pc <= 4'd0;
                        else           pc <= pc + 4'd1;
                    end
