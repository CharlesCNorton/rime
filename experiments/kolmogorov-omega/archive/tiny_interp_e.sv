    ...
    always_ff @(posedge clk) begin
        ...
        if (rst) begin
            ...
            if (pc >= PROG_LEN) begin
                ...
                step <= step + 9'd1;
                case (opcode)
                    3'd0: begin a <= a + 8'd1;            pc <= pc + 4'd1; end  // INC (not involution)
                    3'd1: begin a <= 8'd0 - a;            pc <= pc + 4'd1; end  // NEG (involution)
                    3'd2: begin a <= b; b <= a;           pc <= pc + 4'd1; end  // SWP (involution)
                    3'd3: begin a <= ~a;                  pc <= pc + 4'd1; end  // CPL (involution)
                    3'd4: begin a <= a ^ b;               pc <= pc + 4'd1; end  // XOR (involution w/ fixed B)
                    3'd5: begin                                       // JNZ
                        if (a != 8'd0) pc <= 4'd0;
                        else           pc <= pc + 4'd1;
                    end
