...
module tiny_interp_3r (
    ...
    always_ff @(posedge clk) begin
        ...
        if (rst) begin
            running <= 1'b0;
            a <= 8'd0; b <= 8'd0; c <= 8'd0; pc <= 4'd0; step <= 9'd0;
            result <= 8'd0; halted <= 1'b0;
        end else if (start) begin
            running <= 1'b1;
            a <= init_a; b <= init_b; c <= 8'd0; pc <= 4'd0; step <= 9'd0;
            halted <= 1'b0;
            ...
            if (pc >= PROG_LEN) begin
                ...
                step <= step + 9'd1;
                case (opcode)
                    // ISA-3R: INC DEC SWP ROT ADD JNZ (3-register coupling)
                    3'd0: begin a <= a + 8'd1;                    pc <= pc + 4'd1; end  // INC
                    3'd1: begin a <= a - 8'd1;                    pc <= pc + 4'd1; end  // DEC
                    3'd2: begin a <= b; b <= a;                   pc <= pc + 4'd1; end  // SWP A<->B
                    3'd3: begin a <= c; c <= b; b <= a;           pc <= pc + 4'd1; end  // ROT A<-C<-B<-A
                    3'd4: begin a <= a + b;                       pc <= pc + 4'd1; end  // ADD
                    3'd5: begin                                       // JNZ
                        if (a != 8'd0) pc <= 4'd0;
                        else           pc <= pc + 4'd1;
                    end
