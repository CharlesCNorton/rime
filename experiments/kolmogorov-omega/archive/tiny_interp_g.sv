    ...
    always_ff @(posedge clk) begin
        ...
        if (rst) begin
            ...
            if (pc >= PROG_LEN) begin
                ...
                step <= step + 9'd1;
                case (opcode)
                    // ISA-G: INC SHR MOV SUB AND JNZ — predicted Omega ≈ 0.653 (same coupling as D)
                    3'd0: begin a <= a + 8'd1;            pc <= pc + 4'd1; end  // INC
                    3'd1: begin a <= {1'b0, a[7:1]};     pc <= pc + 4'd1; end  // SHR
                    3'd2: begin b <= a;                   pc <= pc + 4'd1; end  // MOV
                    3'd3: begin a <= a - b;               pc <= pc + 4'd1; end  // SUB
                    3'd4: begin a <= a & b;               pc <= pc + 4'd1; end  // AND
                    3'd5: begin                                       // JNZ
                        if (a != 8'd0) pc <= 4'd0;
                        else           pc <= pc + 4'd1;
                    end
