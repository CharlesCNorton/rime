module tiny_interp_w16 (
    input wire clk,input wire rst,input wire start,
    input wire [23:0] program,input wire [15:0] init_a,input wire [15:0] init_b,
    input wire [23:0] isa_lut,
    output logic [15:0] result,output logic done,output logic halted);
    localparam integer MAX_STEPS=65536,PROG_LEN=8;
    logic [15:0] a,b;logic [3:0] pc;logic [8:0] step;logic running;
    logic [2:0] raw_op;
    always_comb case(pc)
    ...
    always_ff @(posedge clk) begin done<=0;
        if(rst) begin running<=0;a<=0;b<=0;pc<=0;step<=0;result<=0;halted<=0;end
            ...
            if(pc>=PROG_LEN) begin result<=a;halted<=1;done<=1;running<=0;end
                ...
                12:begin a<={a[14:0],1'b0};pc<=pc+1;end
