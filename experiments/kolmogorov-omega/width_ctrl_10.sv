module width_ctrl_10 #(parameter integer N_INTERP=16) (
    ...
    logic [9:0] interp_result[0:N_INTERP-1];logic interp_done[0:N_INTERP-1];logic interp_halted[0:N_INTERP-1];
    logic [9:0] init_a_reg,init_b_reg;logic [23:0] isa_lut_reg;
    genvar gi;generate for(gi=0;gi<N_INTERP;gi=gi+1) begin:gen_interp
        tiny_interp_w10 I(.clk(clk),.rst(rst),.start(interp_start),
    ...
    (* keep *) logic [10:0] run_timer;(* keep *) logic all_done;
        ...
        if(rst) begin state<=S_IDLE;progs_tried<=0;halt_count<=0;match_count<=0;
            ...
            case(state)
                ...
                S_RUN:begin run_timer<=run_timer+1;all_done=1;for(int i=0;i<N_INTERP;i=i+1)if(!interp_done[i])all_done=0;
                    if(all_done||run_timer>=1034)begin collect_idx<=0;state<=S_COLLECT;end end
                S_COLLECT:begin if(collect_idx<N_INTERP)begin
                    if(interp_halted[collect_idx])begin halt_count<=halt_count+1;
                        if(interp_result[collect_idx]=={(10){1'b0}})match_count<=match_count+1;end
