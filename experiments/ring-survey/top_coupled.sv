// ring-survey (coupled): coupled oscillator dynamics experiment.
// Two ring oscillators with shared routing to measure injection
// locking behavior. See README.md.
module top (
    input  wire       clk,
    input  wire       usb_rx,
    output wire       usb_tx,
    ...
    localparam integer NUM_SMALL = 500;
    localparam integer SMALL_STAGES = 5;
    localparam integer LARGE_STAGES = 1001;
    localparam integer COUPLE_INTERVAL = 50;
    localparam integer MEAS_CYCLES = 125000;
    localparam integer IDX_BITS = 10;
    ...
    wire [NUM_SMALL-1:0] small_out;
    genvar sr;
    generate for (sr=0; sr<NUM_SMALL; sr=sr+1) begin : gs
        (* keep *) wire [SMALL_STAGES:0] r;
        assign r[0] = r[SMALL_STAGES];
        genvar st;
        for (st=0; st<SMALL_STAGES; st=st+1) begin : gst
            (* keep *) LUT4 #(.INIT(16'h5555)) inv(.Z(r[st+1]),.A(r[st]),.B(1'b0),.C(1'b0),.D(1'b0));
        end
        assign small_out[sr] = r[1];
    end endgenerate

    (* keep *) wire [LARGE_STAGES:0] lr;
    assign lr[0] = lr[LARGE_STAGES];
    genvar li;
    generate for (li=0; li<LARGE_STAGES; li=li+1) begin : gl
        if (li % COUPLE_INTERVAL == 0 && li/COUPLE_INTERVAL < NUM_SMALL) begin : coupled
            (* keep *) LUT4 #(.INIT(16'h9999)) cinv(
                .Z(lr[li+1]), .A(lr[li]), .B(small_out[li/COUPLE_INTERVAL]),
                .C(1'b0), .D(1'b0)
            );
        end else begin : uncoupled
    ...
    logic mux_out;
    always_comb begin
        if (meas_idx == NUM_SMALL[IDX_BITS-1:0]) mux_out = large_out;
        else if (meas_idx < NUM_SMALL[IDX_BITS-1:0]) mux_out = small_out[meas_idx];
        else mux_out = 1'b0;
    end
    ...
    always_ff @(posedge sys_clk) begin
        ...
        if (rst) begin ms<=M_IDLE; meas_idx<=0; edge_count<=0; meas_timer<=0; end
            ...
            M_STORE: begin
                ...
                if (meas_idx==NUM_SMALL[IDX_BITS-1:0]) ms<=M_DONE;
    ...
    function automatic [7:0] hdr_char(input [5:0] idx);
        case(idx)
            ...
            2:hdr_char="C"; 3:hdr_char="O"; 4:hdr_char="U"; 5:hdr_char="P";
            6:hdr_char="L"; 7:hdr_char="E"; 8:hdr_char="D"; 9:hdr_char=" ";
            10:hdr_char="R"; 11:hdr_char="I"; 12:hdr_char="N"; 13:hdr_char="G";
            14:hdr_char="S"; 15:hdr_char=13; 16:hdr_char=10;
            default: hdr_char=0;
        endcase
    endfunction
    ...
    always_ff @(posedge sys_clk) begin
        ...
        if (rst) begin os<=O_IDLE; o_idx<=0; str_idx<=0; end
            ...
            O_HDR: if (!tbusy&&!tx_send) begin
                if (str_idx<17) begin tx_byte<=hdr_char(str_idx); tx_send<=1; str_idx<=str_idx+1; end
            ...
            O_LINE: if (!tbusy&&!tx_send) begin
                if (o_idx==NUM_SMALL[IDX_BITS-1:0]) begin
                    case(str_idx)
                        0: begin tx_byte<="L"; tx_send<=1; str_idx<=1; end
                        1: begin tx_byte<=","; tx_send<=1; str_idx<=6; end
                    ...
                    case(str_idx)
                        ...
                        2: begin tx_byte<=hex_char({2'b0,o_idx[9:8]}); tx_send<=1; str_idx<=3; end
            ...
            O_NL: if (!tbusy&&!tx_send) begin
                ...
                if (o_idx==NUM_SMALL[IDX_BITS-1:0]) begin str_idx<=0; os<=O_FTR; end
    ...
    assign led = {hb[23], ms!=M_DONE&&ms!=M_IDLE, meas_idx[9:8], sd_rdy};
endmodule
