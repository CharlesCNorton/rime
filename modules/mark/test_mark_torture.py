#!/usr/bin/env python3
"""Torture test for MARK: MARK: Manufacturing-Artifact Random Key. Silicon PUF for unique board identity.
...
def main():
    fw, exp = gen()
    print(f"MARK torture: {len(fw)} instrs, expected 0x{exp:08X}")
    if "--gen-only" in sys.argv:
        return 0
    from compositor_test import build_module, flash_and_read, restore_rime
    from compositor_template import generate_top_sv
    generate_top_sv("mark", fw, Path(__file__).resolve().parent / "top.sv")
    ok, _ = build_module("mark", fw)
    if not ok:
        print("BUILD FAILED")
    restore_rime()
    return 1
    output = flash_and_read("mark")
