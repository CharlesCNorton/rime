...
module kolmogorov (
    ...
    localparam integer N_INTERP = 10;
    localparam integer PROG_LEN = 6;
    localparam integer BATCH_STEPS = 260;

    // --- Interpreter array ---
    logic        interp_start;
    logic [17:0] interp_prog [0:N_INTERP-1];
    ...
    // --- Base-6 odometer: 6 digits, each 0-5, no division ---
    // The odometer represents the program number of interpreter 0.
    // Interpreter i gets odometer + i (computed by sequential increment).
    logic [2:0] odo [0:5];  // 6 base-6 digits

    // Increment a base-6 number by 1, returns carry-out
    function automatic [18:0] base6_inc(input [2:0] d5, d4, d3, d2, d1, d0);
        logic [2:0] r0, r1, r2, r3, r4, r5;
        logic c;
        r0 = d0 + 3'd1; c = (d0 == 3'd5); if (c) r0 = 3'd0;
        r1 = c ? d1 + 3'd1 : d1; if (c && d1 == 3'd5) begin r1 = 3'd0; c = 1; end else c = 0;
        r2 = c ? d2 + 3'd1 : d2; if (c && d2 == 3'd5) begin r2 = 3'd0; c = 1; end else c = 0;
        r3 = c ? d3 + 3'd1 : d3; if (c && d3 == 3'd5) begin r3 = 3'd0; c = 1; end else c = 0;
        r4 = c ? d4 + 3'd1 : d4; if (c && d4 == 3'd5) begin r4 = 3'd0; c = 1; end else c = 0;
        r5 = c ? d5 + 3'd1 : d5; if (c && d5 == 3'd5) begin r5 = 3'd0; c = 1; end else c = 0;
        base6_inc = {c, r5, r4, r3, r2, r1, r0};
    endfunction

    // Pack 6 base-6 digits into an 18-bit program
    function automatic [17:0] pack_digits(input [2:0] d5, d4, d3, d2, d1, d0);
        pack_digits = {d5, d4, d3, d2, d1, d0};
    endfunction

    // --- State ---
    logic [7:0]  target;
    logic [2:0]  search_len;
    logic [17:0] single_prog;
    ...
    // (* keep *) prevents the synthesizer from constant-folding signals through
    // the module boundary. Without these, synthesis may determine the FSM is
    ...
    function automatic [31:0] prog_space(input [2:0] len);
        case (len)
    ...
    logic [7:0]  load_idx;
    logic [2:0]  cur_d [0:5];  // working copy of odometer during load
    ...
    always_ff @(posedge clk) begin
        ...
        if (rst) begin
            ...
            k_found <= 1'b0; k_value <= 8'd0; k_program <= 18'd0;
            run_done_flag <= 1'b0;
            progs_tried <= 32'd0; batch_num <= 32'd0; match_count <= 32'd0;
            enumeration_done <= 1'b0;
            for (int i = 0; i < 6; i = i + 1) odo[i] <= 3'd0;
        end else begin
            case (state)
                S_IDLE: ;
                ...
                S_LOAD: begin
                    if (load_idx == 0) begin
                        ...
                        for (int i = 0; i < 6; i = i + 1) cur_d[i] <= odo[i];
                    end

                    // Assign current program to interpreter[load_idx]
                    interp_prog[load_idx] <= pack_digits(cur_d[5], cur_d[4], cur_d[3], cur_d[2], cur_d[1], cur_d[0]);

                    // Increment working counter for next interpreter
                    begin
                        logic [18:0] nxt;
                        nxt = base6_inc(cur_d[5], cur_d[4], cur_d[3], cur_d[2], cur_d[1], cur_d[0]);
                        cur_d[0] <= nxt[2:0];
                        cur_d[1] <= nxt[5:3];
                        cur_d[2] <= nxt[8:6];
                        cur_d[3] <= nxt[11:9];
                ...
                S_COLLECT: begin
                    for (int i = 0; i < N_INTERP; i = i + 1) begin
                        if (interp_halted[i] && interp_result[i] == target) begin
                            match_count <= match_count + 32'd1;
                            if (!k_found) begin
                                k_found <= 1'b1;
                                k_value <= search_len;
                                k_program <= interp_prog[i];
                            end
                        end
                    end
            ...
            if (reg_wr) begin
                case (reg_addr)
                    ...
                    12'h014: single_prog <= reg_wdata[17:0];
                    12'h01C: init_a_reg <= reg_wdata[7:0];
                    12'h020: init_b_reg <= reg_wdata[7:0];
                    12'h02C: search_len <= reg_wdata[2:0];
                    default: ;
                endcase
            end
        end
